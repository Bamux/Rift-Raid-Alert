...
def logfile_analysis(logtext):
    try:
        global trigger, language, location, boss
        # Jokes for Siri
        joke = []
        ...
        while True:
            ...
            if log:
                ...
                if 'rift raid alert started' in log:
                    newtrigger = defaulttrigger
                    text = "start rift raid alert"
                if 'rift raid alert stoped' in log:
                    newtrigger = defaulttrigger
                    text = "stop rift raid alert"
                if 'rift raid alert trigger <- keywords off' in log:
                    newtrigger = defaulttrigger
                    zone = ""
                    print(log)
                    text = "stop the search for key words"
                    ...
                    if combattrigger == 1:
                        cut_string = log.split('rift raid alert trigger -> ')
                        new_string = cut_string[1]
                        if zone != new_string:
                            language = "all"
                            location = "all"
                    ...
                    if '[rift raid alert] raidbuff missing' in log:
                        trigger = Weaponstone_Flask_Food
                else:
                    trigger = newtrigger
...
def triggerload(file):  # get parametrs from Rift_Raid_Warnings.ini
    global trigger, stacks, logfile, volume, special, keywords, buffcheck, combattrigger
...
print("Raid Rift Alert Version 0.2.0")
...
Weaponstone_Flask_Food = trigger
